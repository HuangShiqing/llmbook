...
1. **内容生成/改写**：流式 SSE 返回，阅读页底部多轮对话面板交互。支持探讨性对话（AI 正常回复）和内容输出（AI 用 `<content>` 标签包裹，前端提取后显示 diff 预览）。AI 自动获取全书目录结构并定位当前编辑章节
2. **目录调整**：支持多轮对话，可先讨论再输出结构。AI 返回 JSON 代码块时前端解析为新目录并显示 diff 树预览，纯文字回复作为讨论展示。确认后应用（自动创建/删除对应 .md 文件）
